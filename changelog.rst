...
- Bumped the legacy non-Windows packaging Perl runtime to 5.42.2.
...
- Sanitized the legacy non-Windows packaging launch environment and added safer fallback handling.
- Labeled Windows installer runtimes with exact versions.
- Fixed the notification icon and corrected a legacy non-Windows packaging screenshot asset.
...
- Improved behaviour on KDE in legacy non-Windows packaging and enabled Python/Perl plugin support.
...
- Clicking links in the chat window with legacy non-Windows packaged builds now works.
...
- Switched legacy non-Windows packaging/builds to GTK 3 (build dependencies, Meson flags, and package metadata).
- Added legacy package split-install manifests so files are assigned to the correct binary packages.
- Fixed legacy packaging coverage for the man page and MIME XML to avoid unassigned-file (dh_missing) failures.
- Updated legacy packaging CI workflow and artefact upload paths for actions/upload-artifact@v4 compatibility.
...
- Fixed newline issues when syncing server list files across environments.
...
- At that point, legacy non-Windows packaging was still in transition.
- A legacy non-Windows package did not include the Python plugin due to packaging issues (Windows build unaffected).
...
- rename installed icon in legacy non-Windows packaging to match app-id (fixes notification icon)
...
- remove libnotify dependency in legacy non-Windows builds, fixing hangs when notifications are shown
- remove libproxy dependency in legacy non-Windows builds
...
- fix performance regression in non-Windows builds
...
- fix characters getting cut off when their width changes (in non-Windows builds)
...
- fix building sysinfo on legacy desktop targets <= 10.9
...
- add lua plugin
- change desktop launcher behaviour to open URLs in an existing instance on non-Windows builds
- misc chanopt fixes
...
- add support for native Windows 8.1+ notifications
...
- rewrite sysinfo plugin
  ...
  - add support for legacy desktop notification integration
  ...
  - fix handling unsupported CPU architecture on non-Windows targets

- rewrite identd plugin

  - add support for non-Windows targets
...
- improve build options in non-Windows builds to be more secure
...
- fix opening utf8 urls on Windows and legacy desktop targets
...
- fix scrolling the chanview in non-Windows builds
- fix building plugin support on some platforms (legacy desktop 10.8)
- fix FiSHLiM decryption when identify-msg is enabled
- fix expanding *~user* in non-Windows builds
...
- improve legacy desktop integration (custom theme, app menu, dock icon, spell check, app bundle, key bindings)
...
- improvements to sysinfo on non-Windows, including ``-e`` to print info instead of saying
...
- add libcanberra support on non-Windows
...
- add theme manager to non-Windows build system
...
- fix sound directory option on non-Windows
...
- make the source tree compliant with legacy packaging policies
- install SVG icon on non-Windows
- enable Plugin-Tray menu on Windows
- enable IPv6 by default on non-Windows
...
- change default DCC download directory to ~/Downloads on non-Windows
...
- fix legacy packaging spec file
...
- add support for the SysInfo plugin on non-Windows
...
- add manual page on non-Windows
...
- build system cosmetics on non-Windows
...
- fix compilation on Red Hat and legacy packaging
...
- fix desktop notification icons on non-Windows
...
- add support for the theme manager on non-Windows
...
- make libproxy an optional dependency on non-Windows
...
- fix installing/loading plugins on non-Windows
...
- add support for the Checksum, Do At, FiSHLiM and SASL plugins on non-Windows
...
- non-Windows build system fixes and cosmetics
- make Git ignore non-Windows-specific intermediate files
