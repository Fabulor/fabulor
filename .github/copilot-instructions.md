# Copilot instructions for `zoitechat-master`
...
- **`installer\`** — the WiX v4 Burn-based installer, treated as the authoritative,
  ...
  `To-Do.md`, the target installed layout is `ProgramFiles\ZoiteChat\ZoiteChat.exe`
  ...
  registry entries only in Installed mode, and ZoiteChat's own updater (not Windows
  Update or MSIX).
