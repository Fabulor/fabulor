...
static const struct key_action key_actions[KEY_MAX_ACTIONS + 1] = {
	...
	{key_action_menu_shortcut, "Menu Shortcut",
	 N_("Runs one of the built-in menu shortcuts. Set Data 1 to: network-list, new-server-tab, new-channel-tab, new-server-window, new-channel-window, close, quit, menu-toggle, user-list-toggle, fullscreen-toggle, disconnect, reconnect, join-channel, channel-list, away-toggle, reset-marker, move-marker, copy-selection, search-text, search-next, search-previous, contents, project-website, report-issue or about.")},
...
key_save_kbs (void)
{
	...
	if (fd < 0)
		return 1;
	write (fd, buf, g_snprintf (buf, 510, "# Fabulor key bindings config file\n\n"));
