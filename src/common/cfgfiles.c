...
get_xdir (void)
{
	...
	{
#ifndef WIN32
		xdir = g_build_filename (g_get_user_config_dir (), FABULOR_DIR, NULL);
#else
		...
		else
		{
			...
			xdir = g_build_filename (roaming_path, "Fabulor", NULL);
...
default_file (void)
{
	...
	if (!dfile)
	{
		dfile = g_build_filename (get_xdir (), "fabulor.conf", NULL);
	}
	return dfile;
}
...
const struct prefs vars[] =
{
	...
	{"gui_gtk4_variant", P_OFFINT (hex_gui_gtk4_variant), TYPE_INT},
	...
	{"gui_gtk4_theme", P_OFFSET (hex_gui_gtk4_theme), TYPE_STR},
