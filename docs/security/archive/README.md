# Security Review Archive

This directory preserves completed security audits and incident analyses:

- [Manifest plugin security audit](manifest-plugin-disabled-state-audit.md)
- [Enchant Windows crash analysis](enchant-windows-crash-analysis.md)

The records remain useful for threat-model history, regression rationale, and
release provenance. Current security policy belongs in the parent
`docs/security` directory and current implementation guidance belongs with the
relevant plugin, runtime, or architecture documentation.

Archived records should change only for link repair, factual correction, or an
explicit archive-policy update.
